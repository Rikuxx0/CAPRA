...
## Testing Guidelines
Add targeted tests for new parsing or graph logic under `tests/`. Prefer `pytest`, with files named `tests/test_<module>.py`. At minimum, validate:
- Grype JSON/SARIF parsing with representative fixtures
- CVE-to-node mapping behavior
- Draw.io node/edge fact extraction with a small sample
...
## Layer 1 Boundaries
Layer 1 is fact-only normalization. Do not add Attack Operator generation, NVD access, LLM use, path search, exploitability/success decisions, risk or Bayesian scoring, payload generation, or execution. Draw.io and cross-cloud dependency inputs remain optional. Keep important assets as `goal_candidate`; set `is_goal` only from an explicit analysis selection. Preserve source provenance and stable IDs, recursively redact secrets/tokens/credentials in raw evidence, and keep the Layer 1 JSON directly loadable by Layer 2.

## Layer 2 Safety and Reproducibility
Layer 2 is deterministic, rule-based modeling: do not use an LLM, execute attacks, validate a live target, run Bayesian updates, or fetch/store exploit code. Keep Hound semantics isolated by adapter; IAMHoundDog uses bounded same-source multi-edge patterns, while direct attack edges are converted only by their source-specific adapter. Never connect Operators from node equality alone; require matching effects/preconditions or subject-specific produces/requires capabilities. Emit only forward `enables` connections; keep `requires` on the dependent Operator instead of generating a reverse edge. Preserve fact provenance and unresolved data, generate stable IDs from canonical inputs, and recursively redact secrets/tokens from evidence, output, and logs. Runtime validation belongs to Layer 3.
